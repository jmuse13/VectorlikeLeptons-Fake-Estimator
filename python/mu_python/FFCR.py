...
def make_FF_histo_updown(up_down,in_anti,in_id,bins,samples,file_path):
    ...
    outanti = ROOT.TH1D("anti","anti",970,30,1000)
    outid = ROOT.TH1D("id","id",970,30,1000)
    outhisto = ROOT.TH1D(up_down+outtitle[1]+"_"+outtitle[2]+"_"+outtitle[3],up_down+outtitle[1]+"_"+outtitle[2]+"_"+outtitle[3],970,30,1000)
    outanti.Sumw2()
    outid.Sumw2()
    outhisto.Sumw2()
    ...
    outf = ROOT.TFile(file_path+"/VLL_FF_mu.root","UPDATE")
    outhisto.Write()
    outf.Close()
    df.Close()
    for file in files:
...
def make_FF_histo_nominal(in_anti,in_id,bins,samples,file_path):
    ...
    outanti = ROOT.TH1D("anti","anti",970,30,1000)
    outid = ROOT.TH1D("id","id",970,30,1000)
    outhisto = ROOT.TH1D(outtitle[0]+"_"+outtitle[1]+"_"+outtitle[2]+"_"+outtitle[3],outtitle[0]+"_"+outtitle[1]+"_"+outtitle[2]+"_"+outtitle[3],970,30,1000)
    outanti.Sumw2()
    outid.Sumw2()
    outhisto.Sumw2()
    df = ROOT.TFile(file_path+"/data.root","READ")
